...
Node * getCycleStartingNode(Node * head)
{
    ...
    Node * middleNode = next(head, cycleLength);

    Node * startingNode = head;
    while (startingNode != middleNode)
    {
        startingNode = next(startingNode, 1);
        middleNode = next(middleNode, 1);
    }
    return middleNode;
}
...
int main()
{
    ...
    std::cout << getCycleStartingNode(firstList)->value << std::endl;
}
